...
runParsecT p s = fmap fst (unParseResult (unParser p s cok cerr eok eerr))
  where
    cok a s' _  = ParseResult (return (Reply s' Consumed (OK a), undefined))
    cerr err s' = ParseResult (return (Reply s' Consumed (Error err), undefined))
    eok a s' _  = ParseResult (return (Reply s' Virgin   (OK a), undefined))
    eerr err s' = ParseResult (return (Reply s' Virgin   (Error err), undefined))
